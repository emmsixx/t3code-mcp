...
State is stored as **unencrypted JSON**, with directory mode `0700` and file mode `0600`. It contains the bridge's native Clerk client credential, session ID, DPoP private key, and operation journal (including submitted instructions). During login it also stores a pending attempt's IDs, next factor, and expiration; it does not store the submitted email or verification codes. It does not read browser cookies, existing T3 credentials, or `~/.t3/userdata`. Relay/environment access tokens remain in memory; short-lived bootstrap credentials are exchanged immediately.
...
Login releases its lock between prompts or CLI invocations. Run `login-status` to check an existing session or resume a pending email/TOTP step after restarting an agent. Pending attempts have a ten-minute local limit. Run `logout` to discard an expired or interrupted attempt before starting again. [Agent login commands and recovery](agent-setup.md#3-guide-the-user-through-login).
