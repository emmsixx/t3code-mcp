...
export const session = z.object({ status: z.string(), activeTurnId: z.string().nullable(), lastError: z.string().nullable() });
...
export const threadSummary = z.object({ id, projectId: id, title: z.string(), modelSelection, runtimeMode, interactionMode, session: session.nullable(), latestTurn: latestTurn.nullable(), hasPendingApprovals: z.boolean(), hasPendingUserInput: z.boolean() });
